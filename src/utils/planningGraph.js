...
function itemMatchesWorklogDay(item, key) {
  return worklogDates(item).some((date) => dateKey(date) === key) ||
    dateKey(item.createdAt) === key;
}
...
function appendDayHierarchy({
  ...
  pathId,
  groupBySprint = true,
  matchWorklogOnly = false,
}) {
  const dayKey = dateKey(day);
  const matchesDay = matchWorklogOnly ? itemMatchesWorklogDay : itemMatchesDay;
  const matchingIds = new Set();

  sourceItems
    .filter((item) => matchesDay(item, dayKey))
    .forEach((item) => {
  ...
  ordered.forEach((item) => {
    ...
    if (groupBySprint && !sprintNodeByTitle.has(sprintTitle)) {
    ...
    output.push({
      ...
      childWorklogDate: day.toISOString(),
    });
  });
}
...
export function buildTimeGraph({
  ...
  output.push(virtualNode({
    ...
    addChildTypes: viewMode === "day" ? ["epic"] : undefined,
    childParentId: viewMode === "day" ? ROOT_ID : undefined,
    childWorklogDate: viewMode === "day" ? startOfDay(anchor).toISOString() : undefined,
  }));
